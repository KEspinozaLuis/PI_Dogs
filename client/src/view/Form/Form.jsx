...
const Form = () => {
  ...
  const handleChange = (event) => {
    ...
    if(property === 'temperaments'){
      if (!form.temperaments.includes(value)) setForm({ ...form, temperaments: [...form.temperaments, value] });
    }
      ...
      <div className="container">
        <div className={styles.containerForm}>
              ...
              <div className={styles.listTemperament}>
                {
                    form.temperaments.map(temp => 
                        <div className={styles.temperament} key={temp} >
                            ...
                            <button onClick={()=> handleDeleteTemperament(temp)} className={`${styles.close} btn`}><BsXCircleFill/></button>
                        </div> 
                    )
                }
              </div>
          ...
          <div className={styles.preview}>
            <h2>PREVIEW</h2>
            {form.image && <p className={styles.previewImg}><img src={form.image} alt={form.name}/></p>}
            {form.name && <p>Name: <span>{form.name}</span></p>}
            {form.minHeight && form.maxHeight ? <p>Height: <span>{form.minHeight} - {form.maxHeight} cm</span></p> : ''}
            {form.minWeight && form.maxWeight ? <p>Weight: <span>{form.minWeight} - {form.maxWeight} kg</span></p> : ''}
            {form.temperaments.length !==0 && <p>Temperaments: <span>{form.temperaments.join(', ')}</span></p>}
            {form.life_span && <p>Years of life: <span>{form.life_span}</span></p>}
          </div>
        </div>
